...
def creeInstance(n,npuits,i,p1,p2,relief):
    ...
    nbLignesPuits = npuits//n + min(1,npuits%n)
    posPuitsListe = []
    for i in range(nbLignesPuits-npuits%nbLignesPuits):
        posPuitsListe.append(random.sample(range(n),npuits//nbLignesPuits))
    for i in range(npuits%nbLignesPuits):
        posPuitsListe.append(random.sample(range(n),npuits//nbLignesPuits+1))
    for i in range(nbLignesPuits):
        for j in range(len(posPuitsListe[i])):
            courant = posPuitsListe[i][j]
            posPuitsListe[i][j] = n*random.randint(limVersant[courant]-int(1.5*nbLignesPuits)+3*i,limVersant[courant]-int(1.5*nbLignesPuits)+3*i+1)+courant
    posPuits = []
    for i in range(nbLignesPuits):
        posPuits += posPuitsListe[i]
...
def dessinInstanceM(N_noeud,posPuits,limVersant,initial,policy,redirections):
