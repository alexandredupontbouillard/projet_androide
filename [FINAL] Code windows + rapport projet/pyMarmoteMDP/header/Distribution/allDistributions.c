		...
		int Mode )
{
  ...
  case PNED_PRINT_MODE:
    ...
    case 'A':
      ...
      fprintf( OutFile, "Gamma shape %8.4f rate %8.4f ", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
      break;
    case 'E':
      fprintf( OutFile, "exponential mean %8.4f ", The_Law->Parameters[0] );
      break;
    case 'C':
      fprintf( OutFile, "constant value %8.4f ", The_Law->Parameters[0] );
      break;
    case 'B':
      fprintf( OutFile, "bernoulli proba %8.4f ", The_Law->Parameters[0] );
      break;
    case 'g':
      fprintf( OutFile, "geometric proba %8.4f ", The_Law->Parameters[0] );
      break;
    case 'G':
      fprintf( OutFile, "Geometric proba %8.4f ", The_Law->Parameters[0] );
      break;
    case 'I': 
      fprintf( OutFile, "constant infinite value " );
      break;
    case 'U':
      fprintf( OutFile, "uniform low %8.4f high %8.4f", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
      break;
    case 'W':
      fprintf( OutFile, "weibull a %8.4f b %8.4f", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
      break;
    case 'N':
      fprintf( OutFile, "normal mean %8.4f stddev %8.4f", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
      break;
    case 'L':
      fprintf( OutFile, "lognormal mean %8.4f stddev %8.4f", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
      break;
    case 'p':
      fprintf( OutFile, "pareto alpha %8.4f beta %8.4f ", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
      break;
    case 'R':
      fprintf( OutFile, "erlang phases %4.0f mean %8.4f ", 
	       The_Law->Parameters[0], The_Law->Parameters[1] );
    break;
    case 'O':
      fprintf( OutFile, "%c %8.4f ", The_Law->Name, The_Law->Parameters[0] );
      fprintf( OutFile, "%8.4f %8.4f ",
	       The_Law->Parameters[1],
	       The_Law->Parameters[2] );
      break;
    case 'H':
      fprintf( OutFile, "hyperexpo means { ");
      for ( i=1; i<The_Law->Parameters[0]; i+=2 ) {
	fprintf( OutFile, "%8.4f ", The_Law->Parameters[i] );
      }
      fprintf( OutFile, "} probas { ");
    ...
    case 'P':
      fprintf( OutFile, "IPP [ " );
      fprintf( OutFile, "%f, ", The_Law->Parameters[0] );
      Write_Law( OutFile, The_Law->Law_Parameters[0], Mode );
      fprintf( OutFile, ", ");
      Write_Law( OutFile, The_Law->Law_Parameters[1], Mode );
      fprintf( OutFile, " ] ");
      break;
    default:
  ...
  default:
    switch( The_Law->Name ) {
    case 'I':
      fprintf( OutFile, "%c ", The_Law->Name );
      break;
    case 'E':
    case 'C':
    case 'B':
    ...
    case 'L':
    case 'p':
      fprintf( OutFile, "%c %8.4f %8.4f ", 
	       The_Law->Name,
	       The_Law->Parameters[0],
	       The_Law->Parameters[1] );
      break;
    case 'R':
      fprintf( OutFile, "%c %4.0f %8.4f ", The_Law->Name,
	       The_Law->Parameters[0],
	       The_Law->Parameters[1] );
      break;
    case 'O':
...
{	
  ...
  case 'l':
    ...
    break;
  case 'g':
    The_Mean = The_Law->Parameters[0] / ( 1.0 - The_Law->Parameters[0] );
    break;
  case 'G':
    The_Mean = 1.0 / ( 1.0 - The_Law->Parameters[0] );
    break;
  case 'U':
    The_Mean = ( The_Law->Parameters[1] + The_Law->Parameters[0] ) / 2.0;
    break;
  case 'R':
...
{	
  ...
  case 'g':
    if ( Order == 2 )
      The_Moment = Param[0] * ( 1.0 + Param[0] ) 
	/ ( 1.0 - Param[0] ) / ( 1.0 - Param[0] );
    else {
  ...
  case 'I':
    ...
    break;
  case 'U':
    The_Moment = pow( Param[1], 1+Order ) - pow( Param[0], 1+Order );
    The_Moment /= ( Param[1] - Param[0] );
    The_Moment /= ( 1.0 + (double)Order );
    break;
  case 'R':
...
{	
  ...
  case 'A':
    ...
    break;
  case 'C':
    The_Value = exp( -s * Param[0] );
    break;
  case 'E':
    The_Value =  1.0 / ( 1.0 + s * Param[0] );
    break;    
  case 'B':
    The_Value = 1.0 - ( 1.0 - exp( -s ) ) * Param[0];
    break;
  case 'g':
    The_Value = ( 1.0 - Param[0] ) / ( 1.0 - Param[0] * exp( -s ) );
    break;
  case 'G':
...
{	
  ...
  case 'A':
    ...
    break;
  case 'C':
    The_Value = - Param[0] * exp( -s * Param[0] );
    break;
  case 'E':
    The_Value =  - Param[0] / ( 1.0 + s * Param[0] ) /
      ( 1.0 + s * Param[0] );
    break;    
  case 'B':
    The_Value = - exp( -s ) * Param[0];
    break;
  case 'g':
    The_Value = - ( 1.0 - Param[0] ) * Param[0] * exp( -s )
      / ( 1.0 - Param[0] * exp( -s ) )
      / ( 1.0 - Param[0] * exp( -s ) );
    break;
  case 'G':
  ...
  case 'U':
    The_Value = ( s == 0.0 ?
		  ( Param[1] - Param[0] ) / 2.0 :
		  ( 1.0 + s * Param[0] ) * exp( -s * Param[0] ) - 
		  ( 1.0 + s * Param[1] ) * exp( -s * Param[1] ) / s / s /
		  ( Param[1] - Param[0] ) );
    break;
  case 'R':
    The_Value =  Param[0] / ( s*Param[1] + Param[0] );
...
{	
  ...
  case 'U':
    if ( x <= Param[0] )
      ...
      The_Value = ( x > Param[1] ? 0.0 :
		    ( Param[1] - x ) / ( Param[1] - Param[0] ) );
    break;
    /* laaaaazy
       case 'R':
       The_Value = Param[0] / ( s*Param[1] + Param[0] );
